...
설치가 갈리는 지점은 OS가 아니라 **그 기기의 업로드가 한 번에 얼마나 나가는가**입니다 —
아티팩트 발행은 호 전체를 한 번에 올리므로, 큰 호만 전송 중 끊기는 일이 생깁니다.
설치할 때 `tools/check_uplink.py`(E9)가 이것을 **호를 만들기 전에** 판정합니다.

발행이 반복해서 끊기면 → **[docs/troubleshooting-network.md](docs/troubleshooting-network.md)**
증상별 분기·실측표·무죄 목록·진단 규율이 그 문서에 있습니다. 첫 동작은 이 한 줄입니다:

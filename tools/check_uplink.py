...
왜 이 게이트가 있는가 (2026-08-16 맥 워커 규명 · 같은 날 원인 정정):
  맥 워커가 두 주 넘게 「API 연결 끊김」으로 발행을 못 했다. 원인은 회선도 NIC 도
  TCP 도 PMTU 도 아니었다 — 512KB 까지는 8.6MB/s 로 올라가다가 그 위부터 무너지고,
  무제한 전송은 15.0초에 ECONNRESET 로 죽었다. 같은 파일을 늦춰 흘리면 완주했다.
  즉 **대역이 모자라서가 아니라 감당보다 빨리 밀어서** 끊긴 것이다.

  **그 「빨리」의 주체는 경로가 아니라 송신 측이었다** (A/B/A 왕복 + ECN 통제로 확정):
  macOS 의 TCP 송신 버퍼 자동 확장(`net.inet.tcp.autosndbufmax`, 기본 4MB)이 범인이다.
  그 기기에 셰이퍼는 없었다 — dnctl 파이프 공백 · pf dummynet 카운터 73일간 0 ·
  방화벽 비활성 · 공유기에 기기별 제한 기능 부재. 4MB→256KB 로 낮추자 1·2·3·10MB 가
  전부 9.5~12.0MB/s 로 완주했고, 4MB 로 되돌리자 증상이 재발했다.
  경쟁 가설이던 **Tahoe 26.x ECN 업로드 스톨**(Apple 커뮤니티 보고, 같은 OS·같은 기종)은
  **배제됐다** — `ecn_initiate_out=0` 으로 고정한 채 버퍼만 4MB↔256KB 로 오갔더니
  증상이 버퍼를 따라 켜지고 꺼졌다. TSO 는 버스트를 증폭할 뿐 대안 원인이 아니다.

  왜 하필 이 기기인가는 **아직 가설이다.** 확인된 사실만 적으면: xnu 소스에서 송신
  버퍼 자동확장에는 RTT 항이 없다(수신 측만 rcv_srtt 로 BDP 를 본다). 그래서 목적지
  RTT 가 4~5ms 여도 버퍼는 BDP 와 무관하게 4MB 를 향해 자란다. 그 과잉 비율이 저지연
  경로에서 극단적이 된다는 것이 우리 추론이나, **이를 뒷받침하는 공개 문헌은 없고**
  오히려 ESnet 은 저지연 연결이 이런 문제에 덜 노출된다고 서술한다. 단정하지 않는다.

  그래서 이 게이트는 증상 이름을 **「셰이퍼」로 단정하지 않는다** — 같은 사다리 모양이
  경로의 셰이퍼에서도, 송신 측 버스트에서도 나온다. 둘을 가르는 절차는 D8 ⑷ 다.

  이 실패는 발행 시점에만 드러나고, 그때는 이미 호를 다 만든 뒤다. 게다가 증상이
  「연결 끊김」이라 회선 탓으로 오진되기 쉽고, 오진의 결말은 호 크기를 깎는 것
  (D8 이 경계하는 바로 그 행동 — 킷 판형을 한 기기의 사정에 맞춰 내리는 것)이다.
  그래서 판정을 **생산 전으로 당긴다**.
...
판정:
  ...
  ③ 실패의 성격 구분 — 목표가 못 가면 실패(1)이되, **버스트 붕괴인지 좁은 상행인지**를
                      가려서 보고한다. 작은 단이 빠른데 큰 단이 무너지면 버스트 붕괴이고,
                      무너지기 직전 크기가 버스트 허용량·무너진 뒤 속도가 지속 상한이다.
                      전 구간이 고르게 느리면 그때는 실제로 상행이 좁은 것이다

종료코드 (이슈 #37 3분법):
  0  통과 — 목표 크기 완주
  1  판정 결과 실패 — 버스트 붕괴 또는 좁은 상행으로 목표를 못 올림
  2  판정 불가 — 최소 단(64KB)조차 못 보냈다. 기기가 아니라 판정 자체가 성립 안 한 것
                (망 단절·종단 장애·차단). 통과도 아니고 「실패」와도 다르다
  64 사용법 오류 (EX_USAGE)

게이트의 정직성(F5): 이 게이트는 **증상만** 판정한다 — 붕괴가 있는지, 버스트 허용량이
얼마인지까지다. 그 붕괴가 송신 측 버퍼 탓인지, 이 기기의 커널 셰이퍼(dummynet/pf)
탓인지, 망의 QoS 탓인지는 여기서 답하지 않는다 (뒤 둘은 관리자 자격이 필요하다).
소재 판별 절차는 publish-checklist D8 ⑷ 에 있다.
"""
import io
import os
import ssl
...
def diagnose(rows, target_bytes):
    ...
    if is_shaper:
        out.append("실패: %s 는 못 올린다 — **버스트 붕괴**다 (대역 부족이 아니다)" % human(target_bytes))
        out.append("  이 기기는 %s 까지 %s 로 올렸다 — 버스트 허용량이 그만큼이다" % (human(burst), rate(best)))
        if collapsed:
            out.append("  그 위부터 %s 로 무너진다 — 이것이 지속 상한이다" % rate(collapsed))
        if stalled:
            out.append("  %s 는 아예 완주하지 못한다 (%s) — 버스트가 소진된 뒤 드롭된다"
                       % (human(stalled[0]["size"]), stalled[0]["err"] or "끊김"))
        out.append("  대역이 모자란 게 아니라 감당보다 빨리 밀어서 끊긴다. 원인은 둘 중 하나다:")
        out.append("  ⓐ **송신 측 버스트** — macOS 는 이쪽이 확정 사례다. 자격 없이 즉시 확인:")
        out.append("       sysctl net.inet.tcp.autosndbufmax   # 4194304(4MB)이면 이것부터 의심")
        out.append("       sudo sysctl -w net.inet.tcp.autosndbufmax=262144   # 낮추고 재판정")
        out.append("  ⓑ **경로상 셰이퍼** — 기기 커널(dummynet/pf) 또는 망 QoS. D8 ⑷ 로 가른다.")
        out.append("  **호 크기를 깎지 마라** — 원인을 없애는 것이 정공법이다 (D8 ⑷: 소재 판별).")
    else:

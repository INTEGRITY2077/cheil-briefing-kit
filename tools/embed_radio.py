...
- **비트레이트·채널·호 상한은 config 의 `tts.embed` 가 정본이다** (하드코딩 금지).
  종전에는 96k 가 코드에 박혀 있었는데, 3분 20초 대본이 base64 팽창(4/3)을 거쳐
  호 3.28MB 가 되면서 **아티팩트 공개 공유가 거절됐다** (2026-08-12 실측 — 거절 문구
  "This version can't be shared publicly…" 는 크기를 말해 주지 않아 원인처럼 보이지
  않는다). 같은 음성을 64k 모노로 내리면 호 2.14MB 다. config 가 없으면 64k/1ch 로
  돈다 — 종전 기본값 96k 로 되돌아가지 않는다.

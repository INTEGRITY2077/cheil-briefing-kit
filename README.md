...
아니라, 공유기를 나가는 지점에서 그 기기 하나만 조여져 있었다는 뜻입니다(0.5~0.9Mbps
라는 값이 QoS 상한처럼 떨어지는 것도 정황).

발행이 반복해서 끊기면 순서대로 보세요:
1. **다른 기기와 비교** — 같은 공유기의 다른 PC 로 같은 업로드를 재보세요. 거기가
   빠르면 회선이 아니라 그 기기의 문제입니다.
2. **공유기의 기기별 제한** — QoS·트래픽 관리·자녀보호에 그 기기의 MAC/IP 로 걸린
   업로드 상한이 있는지. 이번 실측의 유력 원인입니다.
3. **랜포트·케이블 교체 후 재측정**.
4. 위 셋이 전부 정상인데도 느리면 **그때** 호 크기(`config.yaml` 의 `tts.embed`)를

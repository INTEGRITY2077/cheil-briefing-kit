...
설치 환경에 따라 갈리는 것은 OS가 아니라 **그 기기에 걸린 업로드 셰이핑**입니다 —
아티팩트 발행은 호 전체를 한 번에 올리므로, 셰이퍼가 있으면 큰 호만 전송 중 끊깁니다.
...
**버스트 허용량을 가진 토큰 버킷**이었습니다.

발행이 반복해서 끊기면 순서대로 보세요. **1~3 은 관리자 자격 없이 할 수 있고,
1 번이 원인의 성격을 곧바로 알려줍니다.**

1. **크기 사다리로 셰이퍼를 드러냅니다** — 같은 종단에 64KB · 256KB · 512KB · 1MB · 2MB 를
   차례로 올려보세요.

   ```bash
   for f in 64k 256k 512k 1m 2m; do curl -o /dev/null -w "$f: %{speed_upload} B/s\n" -X POST --data-binary @up-$f.bin https://speed.cloudflare.com/__up; sleep 2; done
   ```

   작은 것은 빠른데 특정 크기부터 무너지면 **셰이퍼**입니다. 그 기기의 회선·랜카드는
   결백합니다 — 무너지기 직전 크기가 곧 **버스트 허용량**이고, 무너진 뒤 속도가
   **지속 상한**입니다 (실측: 512KB 까지 8.6MB/s, 1MB 부터 60KB/s, 30초 쉬면 회복).
   전 구간이 고르게 느리면 셰이퍼가 아니라 실제로 상행이 좁은 것입니다.
2. **총량인지 연결당인지** — 업로드 3개를 동시에 띄웁니다. 합이 단일과 같으면
   (실측 117KB/s vs 112KB/s) 그 기기에 **총량**이 걸린 것입니다.
3. **유선·무선을 동시에** — 두 인터페이스는 MAC 도 IP 도 다릅니다. 그런데도 합이
   단일과 같으면(실측 111KB/s) 버킷이 **MAC/IP 가 아니라 기기 단위**로 잡혀 있다는
   뜻이라, 랜선을 바꾸거나 IP 를 바꿔도 소용없습니다.
4. **셰이퍼의 소재를 가릅니다** — 여기부터는 자격이 필요합니다.
...
5. 위가 전부 정상인데도 느리면 **그때** 호 크기(`config.yaml` 의 `tts.embed`)를

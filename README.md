...
설치 환경에 따라 갈리는 것은 OS가 아니라 **그 기기의 상행 대역**입니다 — 아티팩트
발행은 호 전체를 업로드하므로, 상행이 좁으면 큰 호가 전송 중 끊깁니다.

**「회선이 좁다」로 단정하지 마세요.** 2026-08-15 실측에서 맥미니가 3MB 호를 못 냈지만,
**같은 공유기 아래 다른 PC 는 같은 순간 같은 종단에 9.9MB/s 로 올렸습니다.** 그 맥은
LAN 양방향 11.7MB/s · 인터넷 하행 6.2MB/s 로 멀쩡했고, 링크도 기가비트 full-duplex 에
오류·충돌 0 이었습니다 — **인터넷 상행만 65~112KB/s** 였습니다. 회선도 케이블도 기기도
아니라, 공유기를 나가는 지점에서 그 기기 하나만 조여져 있었다는 뜻입니다.

발행이 반복해서 끊기면 순서대로 보세요. **1·2번은 공유기 로그인 없이 할 수 있고,
2번이 결정적입니다.**

1. **총량 제한인지 확인** — 그 기기에서 업로드를 3개 동시에 띄워보세요. 합이 하나였을
   때와 같으면(실측 117KB/s vs 112KB/s) 연결마다 느린 게 아니라 그 기기에 **총량**이
   걸린 것입니다.
2. **정상 기기의 회선을 빌려 재측정** — 같은 파일을 다른 PC 의 인터넷으로 우회시킵니다.

   ```bash
   ssh -R 9443:speed.cloudflare.com:443 <느린기기> "curl -o /dev/null -w '%{speed_upload}\n' --resolve speed.cloudflare.com:9443:127.0.0.1 -X POST --data-binary @/tmp/up-2m.bin https://speed.cloudflare.com:9443/__up"
   ```

   빨라지면(실측 52KB/s → 6.36MB/s, **122배**) 그 기기의 랜카드도 TCP 설정도 결백합니다.
   제한은 그 기기의 패킷이 **공유기 밖으로 나가는 지점**에 있습니다.
3. **공유기의 기기별 제한 해제** — QoS·트래픽 관리·자녀보호에서 그 기기의 MAC/IP 로
   걸린 업로드 상한을 찾아 푸세요.
4. **랜포트·케이블 교체 후 재측정**.
5. 위 넷이 전부 정상인데도 느리면 **그때** 호 크기(`config.yaml` 의 `tts.embed`)를

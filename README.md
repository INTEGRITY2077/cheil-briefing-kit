...
> 2. **헬스체크 먼저** — clone 전에 현 환경을 판별해 표로 보여준다:
>    Claude Code 세션인가(Bash·파일 도구) · `python --version`(3.10+) · `git --version` ·
>    디스크 1GB · 스케줄 도구 유무. 미충족이면 해결 방법을 안내하고 멈춘다.
> 3. **설치 의사를 묻는다** — "설치할까요?" 와 함께 결정 3가지를 한 번에:
>    설치 위치(기본: 문서 폴더 아래 `briefing-kit`) · TTS 엔진(기본: supertonic, 키 불필요) ·
>    루틴 시각(기본: 생성 07:00 / 배포 07:45).
> 4. **동의하면** clone 후 `SETUP.md` 를 순서대로 수행한다. 이후의 역할 규칙은
...
Claude가 이 README의 진입 절차를 읽고 소개 → 헬스체크 → 설치 여부 질문까지
알아서 진행합니다. 더 명시적으로 하려면:

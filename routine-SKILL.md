...
**스윕 전에 큰축 장부를 과제로 읽는다** (2026-08-14, 이슈 #28 — 축→수집 역방향 배선):
`okf/axes.md` 각 축의 「구멍」과 「검증 지점」을 오늘의 수집 과제 목록으로 삼는다 —
날짜가 지난 검증 지점은 그 관측을 우선 수집하고, 수집 후 계보·점수판 갱신까지가
같은 동작이다 (check_ledger ④⑤ 가 등재 완결·점수판 정합을 봉인한다).
...
- Sweep 단계: 채널별 에이전트 병렬 — 브랜드브리프 / 매드타임스 / DART / 경쟁사 IR 원문
  / 한경 컨센서스 / 글로벌 지주 4사 / **글로벌 광고주 수요측(global-demand)** /
  **AI 지면 공급측·조사(ai-ad-supply)** / 종합 검색(컨퍼런스·계열 물량·주가).
  세계 층 두 채널은 2026-08-14 상시 편입(이슈 #28) — 프로파일 sources 가 정본이다.
  각 에이전트는 structured output(제목·사실·날짜·출처·등급·층/축·**큰축 소속 후보**)으로
  반환하고, 이미 원장에 있는 사실은 제외한다
...
- **① 계약금 — 화두 가설 심사** (문장 2~3줄, 조판물 없음). 첫 질문은 큰축 대조다:
  ...
  루브릭 R6). **생성 규칙**: 후보는 주요 사실 문장(원장 문면)을 줄인 것에서만
  출발한다 — 분석 요약·개념 배열·은유 제조 금지, 실물 명사 우선, 간명 상한(공백
  제외 30자·2절)은 headline-guard brevity 가 기계 판정 (2026-08-14 편집장 반려).
  낙점이 아니라 허가 — 후보 병행 가능, 형은 ②까지 유동.
- **② 중도금 — 뼈대 조판** (MD 30~50줄: 화두+스탠드퍼스트+절 리드+함의.
  판형 정본은 `templates/proto-skeleton.md`, 일일 뼈대는 `eval/proto-YYYY-MM-DD.md`
  로 저장한다 — eval/ 은 로컬 운영 산출물이라 배포되지 않는다, 이슈 #26).
  정성 심사 3렌즈는 여기서 돈다 — **독립 채점이다**: 워크플로우 가능 환경은 병렬
  (심사자당 에이전트, 서로 기록 비공개), 아니면 순차라도 앞 렌즈의 표를 열지 않는다.
  심사자 프롬프트 정본은 `templates/review-prompts.md` (이슈 #29). 미달이면 뼈대
  수정·재심사(분 단위). 작은축(절)마다 소속 큰축을 선언한다(IG5) —
  무소속 절은 필요성 재평가.
- **③ 잔금 — 지면 조판 1회.** 정량 게이트만. 유일한 정성 질문은 「조판이 뼈대를
  배신했는가」 — 배신 수리는 조판 몫, 뼈대를 고치려면 ②로 회귀한다.
정성 심사(IG4)는 시범 운용 — **이원 배선**이다 (이슈 #23 문안 통일): 봉인 위반
(기록 부재·미완결·지적 미처리·미달 후 재심사 부재)은 5절 check_review 종료코드 0
필수로 **차단**, 임계 미달 자체는 비차단 — 재심사 기록 후 발행하되 보고에
「정성 미달 N점」 명기. 임계 시작값 7.0(config review.threshold), 임계 확정과
미달의 차단 승격은 편집장 결정.

# 4. 산출물 생성
**입력은 3b 를 통과한 뼈대다** — 잔금 조판은 패널 임계를 통과한 뼈대 MD
(`eval/proto-YYYY-MM-DD.md`)를 받아 시작하고, 뼈대 없이 지면을 쓰지 않는다
(2026-08-14, 이슈 #30 — 이 절은 3단 신설 이전 문면이라 인계 문장이 없었다.
조판이 뼈대의 화두·함의 문장을 보존했는지는 check_insight ⑨가 대조한다).

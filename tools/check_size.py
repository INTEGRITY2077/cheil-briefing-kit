...
def main():
    ...
    if total <= limit:
        # 꼬리 문구는 config 주석·이 파일 문두에 이미 있던 문장을 **출력으로 올린 것**이다
        # (2026-08-15, 이슈 #37 ④): 실행자는 「통과」 한 줄만 보고 다음 단계로 가므로,
        # 상한이 검증된 임계가 아니라는 사실이 지면(주석)에만 있으면 전달되지 않는다.
        print(f"통과: 상한 대비 {total/limit*100:.0f}% "
              "— 통과가 발행·공유를 보장하지 않는다. D8 은 편집 규율이다 "
              "(상한은 검증된 임계가 아니다 · 정본 판정은 발행 후 E3·E6)")
        return
    ...
    if audio_b64 > 0:
        # 지면을 그대로 두고 상한에 맞추려면 MP3 를 얼마로 내려야 하는지 계산해 준다

---
name: cheil-daily-briefing
description: 제일기획 일일 브리핑 — 생성·배포 시각은 config schedule 이 정본 (기본 07:00 생성, 07:45 배포)
---

제일기획 일일 브리핑을 생성한다.

# 실행 주체 (2026-08-12 확정 — 현존 세션 우선)
루틴의 **1차 실행자는 컨텍스트를 쥔 현존 세션의 세션 크론**(06:57)이다 — 새 세션은
권한·컨텍스트·중복 문제를 반복해서 일으켰다 (08.12 실측: 권한 정지, 이중 생산).
이 스케줄드 태스크(07:07, 새 세션)는 **예비**다: 현존 세션이 꺼져 있거나 크론이
만료(세션 크론은 7일 자동 만료 — 매주 재등록 필요)된 날만 실제로 생산하게 되며,
그 판별은 0-a 단일 생산자 규칙이 한다 (오늘자 산출물이 있으면 검증만 하고 종료).

# 시간표 — 생성과 배포를 나눈다 (시각은 config 가 정본, 하드코딩 금지)

시각은 **config.yaml `schedule`** 에서 읽는다 — `daily`(생성 시작 크론)와
`publish_at`(배포 시각). 이 값은 설치 때 사용자와 합의해 정한 것이고, 언제든
사용자와 재합의해 config 를 고치면 다음 실행부터 그 시각을 따른다.
디폴트는 **생성 07:00 / 배포 07:45** 다. 아래 본문의 07:45 표기는 전부
"config 의 publish_at" 을 뜻하는 디폴트 예시로 읽는다.

- **생성 시작(schedule.daily, 기본 07:00)** — 스케줄러 지연으로 실제로는 몇 분 뒤에 뜬다. 0~6절: 수집, 원장, EVAL, 산출물, 게이트, 음성 준비
- **배포(schedule.publish_at, 기본 07:45)** — 4b절(공유 버전 재지정 + 로그아웃 검증)은 **publish_at 이전에 수행하지 않는다**

공유 시점 우선순위 (2026-08-12 확정 — 한 문장): **아티팩트 발행(새 URL 생성) 자체를
publish_at 이후로 미루고, 발행 직후 즉시 Share → Anyone with the link 를 켠다.**
생성 단계(0~6절)는 로컬 산출물(HTML·오디오·대본)과 게이트까지만 하고 발행은 하지 않는다.
이렇게 하면 '중간 버전 노출 금지'(이 절)와 '발행 직후 공유 ON — 안 켜면 404'(4b ②·E1)가
충돌하지 않는다: publish_at 이전에는 발행 자체가 없으므로 노출될 중간 버전도 없다.

이유: 생성 도중의 중간 버전이 공유 링크에 걸리면 독자가 미완성본을 본다.
publish_at 전에 모든 산출이 끝났으면 남은 시간을 재서 대기한 뒤 배포한다
(`python` 으로 publish_at 까지 남은 초를 계산해 sleep).
publish_at 이 지났는데도 게이트가 안 끝났으면 배포를 미루고 그 사실을 보고에 남긴다 —
미완성본을 정시에 내보내지 않는다. 늦는 것이 틀리는 것보다 낫다.

# 킷 위치
{{KIT_ROOT}}
(설치 시 SETUP 4-3이 이 자리를 실제 설치 경로로 치환한다. {{KIT_ROOT}} 가
그대로 남아 있으면 설치가 끝나지 않은 것이다 — 루틴은 시작하자마자 이 값이
실존 디렉토리인지 확인하고, 아니면 생성하지 말고 그 사실만 보고한다.)
(설치 시 SETUP 4-3이 이 자리를 실제 설치 경로로 치환한다. {{KIT_ROOT}} 가
그대로 남아 있으면 설치가 끝나지 않은 것이다 — 루틴은 시작하자마자 이 값이
실존 디렉토리인지 확인하고, 아니면 생성하지 말고 그 사실만 보고한다.)
(설치 시 SETUP 4-3이 이 자리를 실제 설치 경로로 치환한다. {{KIT_ROOT}} 가
그대로 남아 있으면 설치가 끝나지 않은 것이다 — 루틴은 시작하자마자 이 값이
실존 디렉토리인지 확인하고, 아니면 생성하지 말고 그 사실만 보고한다.)

# 파일 명명 규약 (2026-08-12 확정 — 한 호 = 한 파일)
산출물은 날짜가 곧 이름이다. 수식어(특별판·확정판·N장·gemini 등) 금지:
- 웹판: `output/web/YYYY-MM-DD.html`  · 대본: `output/script/YYYY-MM-DD.md`
- 오디오: `output/audio/YYYY-MM-DD.wav` · 슬라이드: `output/ppt/YYYY-MM-DD.pptx`
- URL 기록: `output/artifact-url-YYYY-MM-DD.txt` (웹판 1줄) — 슬라이드 URL 은
  **별도 파일** `output/artifact-url-slides-YYYY-MM-DD.txt` (4c 참조. 두 줄 통합 아님)
버전은 파일 사본이 아니라 **그 파일을 lint(체크리스트)·eval 통과 후 제자리 갱신**으로
관리한다. 사본·별칭·아카이브 접두어 파일을 만들지 않는다 — 발행된 아티팩트가
버전 이력을 이미 보존한다. 어제 파일은 손대지 않는 것으로 아카이브가 된다.

`config.yaml`을 먼저 읽는다. 경로·검토자·TTS·보존 정책이 거기 있다.
**실행 모드도 거기 있다** — `execution.stages` 가 단계별 직렬/워크플로우 배분의 정본이다:
수집 스윕·팩트체크 검증·심층 감사는 workflow(독립 작업 팬아웃), 집필·게이트·발행은
serial(순서 의존·단일 맥락). workflow 단계는 도구가 있으면 그대로, 없으면 순차 강등
+ 보고 기록(1절 참조). 이 지도에 없는 새 작업이 생기면 같은 원칙으로 판정한다 —
"서로 몰라도 되는 일이 여럿인가(→workflow), 앞 결과가 뒤 입력인가(→serial)".
아래 지시에 절대경로를 쓰지 않는다. config의 상대경로를 킷 루트에 붙여 쓴다.
이 문서의 `python` 명령 표기는 인터프리터 자리표시다 — **macOS·Linux 는 `python3` 로
읽는다** (#33 실측: macOS 에 python 명령이 없다. allowlist 도 두 표기 병기).

# 0-원장. 실행 원장 기록이 첫 동작이다 (2026-08-12 이슈 #9 — 무증거 실행에서)
config 읽기·0-a 확인보다도 먼저, **루틴이 뜨자마자 첫 동작으로**
`output/ledger/run_log.jsonl` 에 시작 1줄을 append 한다 (파일·디렉토리 없으면 만든다):
```
python -c "import json,os,datetime;p='output/ledger/run_log.jsonl';os.makedirs(os.path.dirname(p),exist_ok=True);open(p,'a',encoding='utf-8').write(json.dumps({'event':'start','started_at':datetime.datetime.now().isoformat(timespec='seconds'),'session':'<scheduled-task|session-cron|manual>','mode':'미정'},ensure_ascii=False)+chr(10))"
```
루틴 종료 시(성공이든 실패든) 종료 줄을 append 한다 — 같은 `started_at` 으로 짝을 맞추고
`ended_at`, `mode`(생산/검증/중단), `result`(산출/quiet_day/검증만/실패), `artifacts`(만든
파일 목록), 실패면 `reason` 을 채운다. 같은 줄 재기록이 아니라 **종료 줄 append** 다.
산출을 낸 실행의 종료 줄에는 `gates` 필드 — 실제로 실행한 기계 게이트 목록과 종료코드,
예: `"gates":["check_tables:0","check_exhibits:0","check_css_vars:0","check_ledger:0","check_size:0","check_insight:0","check_formats:0","check_theme:0","check_review:0","check_publish:0"]`
— 를 반드시 기록한다. 종료코드는 **3분법**이다 (2026-08-15 신설, 이슈 #37):
`:0` 통과 · `:1` 판정 결과 실패 · `:2` **판정 불가**(게이트가 물어보지 못했다 — 봇 차단
챌린지·네트워크 오류). `:2` 는 통과가 아니라 `check_run` 이 「판정 불가로 미완결」로
차단하되, 「실패한 게이트를 기록한 채 발행했다」와는 **다른 사유**로 갈린다 — 처방이
산출물이 아니라 판정 경로이기 때문이다(멀쩡한 호에 E5 처방을 내던 오진이 이 이슈다).
`:2` 로 기록한 날은 `reason` 에 무엇이 판정을 막았는지·수동 확인을 했다면 그 시각까지 적는다.
**게이트 기록이 없는 발행은 실패다** — `check_run.py` 가 산출물이
실존하는데 종료 줄의 gates 가 없거나 비어 있으면 실패로 판정한다 (2026-08-13 이슈 #18 —
게이트가 있는데 실행 자체를 건너뛴 채 발행된 사고에서).
산출을 낸 실행의 종료 줄에는 `sources` 필드도 함께 적는다 (2026-08-15 신설) — 그날 실제로
연 상시 소스 id 목록이고, 못 돈 소스는 `"<id>:건너뜀 — 사유"` 로 적는다.
예: `"sources":["dart-target","ir-events","trade-press:건너뜀 — 사이트 장애"]`.
이 필드가 **날짜별 스윕 이력**이다 — 프로파일 `last_checked` 는 소스마다 최신 한 값만
보관하는 현재값 스냅샷이라 오늘밖에 증언하지 못하고, 어제의 스윕 범위는 이 필드에만 남는다
(1절 「갱신은 상시 소스 전수에 대해 한다」 참조).
이 두 줄이 없으면 "안 돈 날"과 "돌았는데 빈손인 날"을 가를 방법이 없다 — 이슈 #9 에서
남은 증거가 scheduled-task 의 lastRunAt 타임스탬프 하나뿐이었다.

# 0-버전. 최신 정책 위에서만 생산한다 (2026-08-13 창설 — CalVer vYYYY.MM.DD[.N])
시작 줄 기록 직후, 생산에 들어가기 전에 `python tools/check_version.py` 를 돌린다.
실패(로컬 ≠ 원격 VERSION)면 **생산을 시작하지 말고** `git pull origin main` 후 재판정한다.
산출물은 워커마다 독립이지만(다른 워커의 호를 참조·병합하지 않는다 — 유착 금지),
게이트·템플릿·규정 같은 **정책은 전 워커가 같은 킷 버전 위에서** 생산해야 한다.
버전 정본은 루트 `VERSION` 한 줄, 변경 내역은 `CHANGELOG.md`. 오프라인·zip 설치본은
경고로 강등되니 그 사실을 보고에 남긴다.

**산출물 부재 = 실패다.** 루틴을 어떤 이유로든 끝낼 때 오늘자 산출물
(`output/web/오늘날짜.html`, 또는 quiet_day 라면 run_log 의 `result: quiet_day` 기록)이
없으면 성공처럼 끝내지 않는다: 종료 줄에 `result: 실패` 와 `reason` 을 쓰고, 보고에도
실패 사유를 남긴다. 판정은 손이 아니라 스크립트로 한다 — 종료 직전
`python tools/check_run.py` (킷 루트에서) 종료코드 0 을 확인하고 결과를 보고에 옮긴다.
전날 실행의 실체 점검은 `python tools/check_run.py --date <전날>` 로 한다.

# 0-a. 단일 생산자 규칙 — 시작하자마자 확인한다 (2026-08-12 이중 생산 사고에서)
`output/web/오늘날짜.html` 또는 `output/artifact-url-오늘날짜.txt`가 이미 존재하면
**오늘자는 이미 다른 세션(수동 작업)에서 발행된 것이다. 생성하지 않는다.**
이 경우 할 일은 검증뿐이다: 발행된 아티팩트가 열리는지, 공유 버전이 최신인지 확인하고
그 결과만 보고한 뒤 종료한다. 같은 날짜의 호를 두 번 만들지 않는다.

# 0. 준비 — 순서대로 읽는다
1. `config.yaml`
2. `profiles/<profile>.yaml`
3. 방송 원장 — config의 `paths.ledger`(기본값: `output/ledger/broadcast_log.jsonl`).
   파일이 없으면 빈 원장으로 새로 만든다. 여기 있는 개념은 다시 넣지 않는다
4. `okf/index.md`
5. `okf/agenda/` 전체
6. `okf/facts/` 중 `status: stable` 인 것

`okf/archive/` 는 읽지 않는다.

# 1. 소스 확인 — 키 없이, 브라우저 패널 없이 돈다

**DART는 RSS로 본다.** `https://dart.fss.or.kr/api/todayRSS.xml`
- XML 50건. `creator` 가 회사명이므로 대상 회사로 거른다
- `pubDate` 가 프로파일의 `last_checked` 이후인 것만 본다
- 공시가 몰리는 날은 50건에서 밀릴 수 있다. 그럴 정황이면 `dsab007/main.do` 목록으로 보완한다
- **보고서 원문 뷰어(`dsaf001/main.do`, `report/viewer.do`)는 열지 않는다.** robots.txt Disallow 대상이다.
  원문이 필요하면 `ir-results` 로 대체한다

나머지 소스 수집은 **다이나믹 워크플로우로 병렬 실행한다** (2026-08-12 실측 —
17개 에이전트 스윕이 순차 수집보다 넓고 깊었다). **이 지시가 곧 워크플로우 사용의
옵트인이다** — 도구가 세션에 있으면 쓴다, 보수적으로 회피하지 않는다.
도구가 목록에 **없으면**(앱 버전·기능 플래그에 따라 다르다) 순차 수집으로 강등하되,
보고 첫머리에 "수집 경로: 순차(워크플로우 도구 부재) — 생성 소요가 평소보다 길다"를
반드시 기록한다. 어느 경로였는지가 보고에 없으면 그 실행은 미완성 보고다:
- Sweep 단계: 채널별 에이전트 병렬 — 브랜드브리프 / 매드타임스 / DART / 경쟁사 IR 원문
  / 한경 컨센서스 / 글로벌 지주 4사 / 종합 검색(컨퍼런스·계열 물량·주가). 각 에이전트는
  structured output(제목·사실·날짜·출처·등급·층/축)으로 반환하고, 이미 원장에 있는
  사실은 제외한다
- **refuter 채널 (상설 1개, 2026-08-15 이슈 #35)**: 위 채널들과 **지시가 반대다** —
  「각 축의 전제를 무너뜨릴 관측만 찾는다. 축을 지지하는 사실은 이 채널의 산출이 아니다.」
  **무엇을** 찾는지의 정본은 `profiles/<profile>.yaml` 의 `macro_axes[].refute_probes`,
  **어디서·얼마나**의 정본은 같은 파일의 `refute_sweep` 블록이다 — 둘 다 **읽어서** 쓴다
  (지침 정본은 프로파일이고 이 문서는 읽으라고만 한다 — 축이 늘거나 지침이 바뀌어도
  SKILL 을 고치지 않는다). `refute_sweep` 이 정하는 것: 어느 상시 소스를 쓰는가(`sources`),
  어떤 어휘로 검색하는가(`query_hint` — 부정 어휘만), 시간창(`window_days`),
  하루 처리량(`probes_per_run`), 프로브 회전 규칙(`rotation`).
  **이게 없으면 매일 명제 14개를 얕게 훑는 형식적 1회전으로 붕괴한다** — 나머지 Sweep
  채널은 전부 소스로 정의되는데 refuter 만 명제로 정의돼 있어 「어디서·얼마나」가 비어
  있었다(2026-08-15 보완). 그날 어디까지 돌았는지는 `okf/axes.md` 「반증 탐색 이력」의
  탐색 범위 칸에 **프로브 번호로** 남긴다 — 그것이 다음 실행의 시작점이다.
  왜 상설인가: 세 축 점수판이 전부 반증 0 이었다 — 반증 재료(챗GPT CPM $60→$25 ·
  OpenAI 자체 목표 대비 10% 페이스 · 빅6 미 광고비 점유율 44.6%→29.6%)가 이미 지면에
  실려 있는데도 반증으로 계상되지 않았다. 축이 옳아서가 아니라 아무도 반증을 찾지 않았기
  때문이다. **그 재료들은 이미 원장에 있다**(F-038·F-051·F-052, 전부 '조건형성' 태그) —
  이 채널은 새 외부 관측을 찾으므로 그것들을 건드리지 못한다. 등재분의 방향 재판정은
  2b 절 ⓪ 소급 재판정이 담당한다
- Verify 단계: 채택 후보 각각을 팩트체커 에이전트가 출처 URL 직접 대조로 검증
  (날짜·숫자·주체 불일치 또는 신규성 미달이면 confirmed=false). 검증 통과분만 원장에 등재
- 워크플로우가 실패하면 WebFetch 순차 수집으로 강등하고 그 사실을 보고에 남긴다
브라우저가 꼭 필요한 소스는 새 탭으로 열고, 실패하면 실패로 기록한다.

**반증 관측의 등재 규칙** (이슈 #35) — 반증도 다른 사실과 **같은 등급·좌표 규율**을 따른다:
`okf/facts/` 미러 + index 재생성 + 등급(1차자료/증권사/보도/관찰) + 축·방향 태그(=반증)까지가
한 동작이다. 반증이라고 낮은 문턱으로 받지 않고, 반증이라고 지면에 먼저 올리지도 않는다.
**반증을 찾지 못한 날은 그 사실 자체를 기록한다** — `okf/axes.md` 축마다 「반증 탐색 이력」에
한 줄(날짜 · 탐색 범위 · 결과). 「안 찾은 날」과 「찾았는데 없는 날」을 가르는 것이 이 기록의
요점이다 — 그 구분이 없으면 반증 0 이 관측 결과인지 미탐색인지 아무도 모른다
(`check_ledger` ⑥ⓒ 가 최근 발행일 기록 부재를 경고한다).

확인 후 프로파일의 `last_checked` 와 `last_item` 을 갱신한다.

**갱신은 상시(`cadence: 매일`) 소스 전수에 대해 한다** (2026-08-15 이슈 #36). 지면을 낸 날
그 소스들이 오늘 돈 기록이 없으면 **발행 시점에 `check_publish`(E6)가 차단하고**, 배포 직후·
다음날 점검에서 `check_run --date <날짜>` 가 다시 본다 (2026-08-15 정정 — 종전 문안 「`check_run`
이 발행을 차단한다」는 과장이었다: `check_run` 은 required-gates 로스터 밖이고 산출물이 있어야
돌아가므로 지면 생성을 막지 못한다. 봉인의 위치를 정확히 적는다. 상세는 publish-checklist IG7).
어느 소스가 상시인지의 정본은 `profiles/<profile>.yaml` 의 `sources[].cadence` 이고, 어휘 집합의
정본은 같은 파일의 `sweep_gate` 블록이다 — 정본에 없는 값은 조용히 빠지지 않고 차단된다.
장애·휴무로 못 돈 소스는 **조용히 두지 말고** `last_item` 을
「`건너뜀 YYYY-MM-DD — 사유 · <종전 항목>`」 형태로 적는다 — 사유는 첫 `·` 앞까지가 사유로
읽히고, 그러면 차단이 아니라 경고로 강등된다(F5 — 정직한 실패는 막지 않는다).
**단 전수를 건너뛰면 차단이다** — 로스터 전부에 사유를 적는 것은 정직한 부분 실패가 아니라
「그날 스윕 0건」이다. 스윕을 못 돌린 날은 지면을 내지 않거나 quiet_day 로 기록한다.

**그리고 그날 연 소스 id 를 run_log 종료 줄의 `sources` 필드에 적는다** (2026-08-15 신설):
`"sources":["dart-target","ir-events",...]`, 못 돈 소스는 `"trade-press:건너뜀 — 사이트 장애"`.
왜 두 군데에 적는가 — 프로파일 `last_checked` 는 소스마다 **최신 한 값만** 보관하는 현재값
스냅샷이라 오늘밖에 증언하지 못한다. 어제 무엇을 돌았는지는 이 필드만이 남긴다(그래서 종전
「다음날 점검 — `--date <전날>`」 은 구조적으로 판정 불가였다 — 오늘 스윕이 이미 스냅샷을
덮었기 때문이다). 이 필드가 있는 날만 소급 판정된다.

스윕 기록은 자기 보고라, 이 봉인이 잡는 것은 「기록조차 안 한 채 좁게 돈 날」이지
「형식적으로 기록만 한 날」이 아니다 — 실제로 무엇을 봤는지는 `eval/YYYY-MM-DD.md` 의 사람 판정이다.

# 2. 원장 갱신
새 사실을 OKF 개념으로 만든다. 형식은 기존 `okf/facts/*.md` 를 따른다.
- `generated: { by: claude/opus-5, at: <ISO8601> }`
- `verified` 는 비운다
- `status: stable`, `stale_after` 는 티어별 기준일
- `grade` 는 1차자료 / 증권사 / 보도 / 관찰 중 하나

SUPERSEDE 는 프로파일의 `supersede.grade_order` 와 `known_chains` 를 따른다.
등급이 높은 자료가 오면 기존 개념을 `deprecated` 로 바꾸고 양방향 링크를 건다.
부분 기간과 누적 기간은 서로 대체하지 않는다.

**JSONL 에 쓴 사실은 같은 실행에서 `okf/facts/` 미러 + index 재생성까지가 한 동작이다 —
`python tools/check_ledger.py` 가 판정한다** (배포 정책 A안, 2026-08-12 이슈 #7 확정).
JSONL 정본(`output/ledger/`)은 gitignore 대상이라 배포되지 않는다 — 미러를 빠뜨리면
배포본에는 실체 없는 참조(F-###)만 남고, 대체된 수치가 유효한 최신값으로 방송된다.
대체(SUPERSEDE)를 수행했으면 대체 대상 파일의 `status: deprecated` + `superseded_by` +
본문 후속 링크, index 「대체됨」 이동까지가 같은 동작의 일부다.

새 일정은 `okf/agenda/` 에 개념으로 만든다.

# 2b. 큰축 증류·재귀납 (2026-08-15 확정, 이슈 #35)

원장 갱신이 끝나면 축을 되본다. 아래 순서가 절차 정본이다 —
**종전의 「소급 재조사는 2026-08-13 끝났고 이후는 증분이다」 문안을 이 절이 대체한다**
(그 문장은 재조사를 영원히 끝난 일로 못박아, 축이 틀렸을 가능성을 절차에서 지웠다 —
2026-08-15 메커니즘 감사 지적).

**⓪ 소급 재판정 — 한 번만, 상설 탐색보다 먼저** (2026-08-15 추가). 상태 정본은
`profiles/<profile>.yaml` `refute_sweep.retro_pass` 이고 `status: 대기` 인 동안은 매 실행
①보다 먼저 처리한다. 왜 있는가 — 이 메커니즘의 신설 근거로 인용한 반증 재료(F-038 챗GPT
CPM $60→$25 · F-051 챗봇 내 광고비 <$1B · F-052 OpenAI 목표 대비 −90%)는 **이미 원장에
있고 전부 '조건형성' 태그다**. 그런데 refuter 채널은 다른 Sweep 채널과 같은 층(「이미 원장에
있는 사실은 제외한다」)에서 **새 외부 관측만** 찾으므로, 진단이 가리킨 대상을 처방이 건드리지
못한다 — 그대로 두면 기존 반증 재료는 조건형성으로 남고 반증 0 은 내일도 그대로다.
하는 일: `retro_pass.targets` 를 그 축의 `refute_probes` 로 다시 읽어 방향 태그를 유지할지
반증으로 옮길지 판정한다(판정은 사람·패널 — 황금률). 결과를 계보 표·점수판·profiles status·
index 요약에 같은 동작으로 반영하고, `retro_pass.status` 를 완료+날짜로 바꾼다. 반증으로
옮긴 축이 생기면 그 축은 곧바로 ②③(재귀납)의 대상이다.

**① 증류 — 축마다 한 문장.** `okf/axes.md` 각 축의 「**현재 판정(증류)**」 슬롯에
그 축의 관측 계보가 **지금 무엇을 말하는지** 한 문장 + 근거 좌표(F-###)를 쓴다.
지면의 추적 줄은 이 슬롯을 읽어 쓴다(원장 표를 다시 훑지 않는다). 문장은 사람·패널이
쓴다 — 기계는 슬롯과 좌표의 실존만 본다(`check_ledger` ⑥ⓐ). 계보가 바뀌지 않은 날은
고치지 않아도 된다. 슬롯 문장에 편집실 어휘(축·점수판·지지/반증)를 그대로 쓰지 않는다 —
그대로 지면에 옮기면 B2 에 걸린다.

**② 재귀납 트리거 판정.** `profiles/<profile>.yaml` 의 `macro_axes[].reinduct_when`
조건을 축마다 대조한다 (문법·지표 정의는 그 블록의 주석이 정본). 조건 하나라도 충족이면
③으로 간다. 기계가 봉인하는 것은 「반증 >= N」 하나뿐이고(`check_ledger` ⑥ⓑ — 충족했는데
재귀납 기록이 없거나 **그 기록이 최신 반증 등재일보다 이르면** 발행 차단. 첫 반증에 한 줄
적은 것으로 이후 반증까지 면제되지 않는다), 나머지(무소속 누적·창마감)는 사람이 읽고 판정한다.
「무소속 >= N」 은 `reinduct_baseline` **이후 새로 등재된** 무소속 판정 원장 행만 센다 —
그 표는 삭제가 없어 누적만 하므로, 기준선 없이 세면 도입 첫날부터 영구 참이 되어 사람이 곧
무시하고 실질 트리거가 「반증 >= 1」 하나만 남는다(2026-08-15 수리). 재귀납을 실제로 돌린
날에는 `reinduct_baseline` 을 그날로 옮긴다.

**③ 충족 시 절차 — 네 단계를 같은 날 끝낸다.**
1. **다렌즈 재독** — `deep_audit` 워크플로우로 그 축의 계보 전건을 자본·구조·수요 렌즈로
   다시 읽는다(수집이 아니라 재독이다 — 새 사실을 찾는 자리가 아니다)
2. **축 판정** — 유지 / 분할 / 합병 / 신설 중 하나를 고르고 사유를 한 줄로 적는다.
   반증이 전제 자체를 무너뜨렸으면 「유지」는 근거를 달아야 하는 선택지다
3. **편집장 승인** — 판정은 승인 전에는 발효하지 않는다. 축의 개폐는 호를 넘어 사는
   추적 질문을 바꾸는 일이라 자동화하지 않는다(황금률)
4. **기록** — `okf/axes.md` 그 축의 「### 재귀납 기록」에 한 줄(날짜 · 촉발 조건 ·
   판정 · 승인). 축을 고쳤으면 `profiles` macro_axes·`okf/index.md` 요약까지 같은 동작으로
   맞춘다(3중 장부 — `check_ledger` ⑤)

# 3. EVAL
`templates/eval-rubric.md` 로 계산해 `eval/YYYY-MM-DD.md` 에 기록한다.
- 8점 이상 → 심층. 대본 4블록 + 아티팩트 갱신
- 3~7점 → 표준. 3블록
- 3점 미만 → 간략

뉴스 선별 우선순위 (2026-08-12 확정 — 주어는 제일기획이다):
1. 제일기획 자체 — 공시·IR·수주·인사·주가
2. 계열 물량 신호 — 삼성전자 제품 출시·판매·마케팅 집행
3. 업계·경쟁사 — 묶어서 1~2건, **헤드라인 불가**.
   경쟁사 실적이 아무리 커도 제일기획 뉴스의 참조물이지 주인공이 아니다
   (2026-08-12 실측: 이노션 IR을 헤드로 올렸다가 반려).

경쟁 지형은 3층으로 본다 (2026-08-12 확정 — 이노션 단일 비교 금지):
- **글로벌 지주**: WPP · 퍼블리시스 · 옴니콤(+IPG) · 덴츠 · 하바스 — 규모·수익모델·AI 전환의 기준선
- **침입자**: 액센추어 송·딜로이트 디지털 등 컨설팅 계열, 광고주 인하우스(버거킹 사례),
  플랫폼 자동화(오픈AI 광고·퍼브매틱 AgenticOS류) — 대행 수요 자체를 잠식하는 축
- **국내**: 이노션 · 대홍 · HSAD · TBWA코리아 · 독립 크리에이티브 — 이노션은 '같은 계열
  광고사 모델'이라 1:1 비교가 유효할 뿐, 유일한 경쟁자가 아니다. 한 회사만 반복 인용하면
  지형이 왜곡된다. 비교를 쓸 때는 어느 층의 비교인지 명시한다.

여기에 **내수/해외 2축을 교차**한다 (2026-08-12 확정): 제일기획은 이익 대부분이 해외에서
나오는 회사다 (1Q 기준 국내 삼성전자 거래 −12%, 북미 +32%, 해외 비계열 비중 32%).
- **내수 축**: 국내 광고 시장 경쟁(이노션·대홍·HSAD·독립·인하우스) + 국내 계열 물량
- **해외 축**: 글로벌 지주·현지 네트워크와의 경쟁 + 해외 계열/비계열 물량 (북미·유럽·중국·동남아)
뉴스와 비교는 어느 축의 이야기인지 명시한다 — 내수 경쟁사 실적으로 해외 사업을,
해외 지주 담론으로 내수 시장을 설명하지 않는다.

# 3b. 편집 3단 — 계약금·중도금·잔금 (2026-08-13 확정, 정본 templates/pipeline-three-stage.md)
비싼 조판을 심사 루프에 두지 않는다 — 심사는 싼 단계에서 다 치르고 조판은 1회다
(실측: 뼈대 심사 루프는 분 단위, HTML 재조판은 시간 단위).
- **① 계약금 — 화두 가설 심사** (문장 2~3줄, 조판물 없음). 첫 질문은 큰축 대조다:
  「오늘 관측이 어느 큰축(profiles macro_axes·okf/axes.md)을 움직였나」. 기각 사유
  소거: 연간 주제 비정렬·무담보 긴박감·내부어·노이즈(소송류)·카피 인테그리티 위반
  (헤드의 명사는 본문·원장 실존 명사만, 독자=의사결정을 이루는 팀 주어 우선 —
  루브릭 R6). 낙점이 아니라 허가 — 후보 병행 가능, 형은 ②까지 유동.
- **② 중도금 — 뼈대 조판** (MD 30~50줄: 화두+스탠드퍼스트+절 리드+함의,
  templates/proto-skeleton.md 판형이 정본). 정성 심사는 여기서 돈다 —
  3렌즈(reader/refuter/crosser) + **cold(배경 0 독자) 렌즈**: cold 에게는 지면
  텍스트만 주고 원장·축 장부·전일 호를 차단한다 — 필자도 심사자도 원장을 읽고
  들어와 생략이 생략으로 안 보이는 전원 오염을 cold 가 깬다 (2026-08-14 실측:
  게이트 전종 초록에서 막힘 25곳 — AAM 미정의·내부어 노출·지시어 미복원.
  프롬프트 정본 templates/review-prompts.md cold 절). cold 의 막힘 목록은 전건
  처리(반영/기각) 후에만 잔금 진입. 미달이면 뼈대 수정·재심사(분 단위).
  작은축(절)마다 소속 큰축을 선언한다(IG5) — 무소속 절은 필요성 재평가.
  생략의 규칙: **생략·지시어는 그 지점까지 읽은 텍스트 안에서 복원될 때만 허용** —
  제목은 선행 문맥 0의 진입점이라 보편 상식으로만 복원 가능해야 한다.
- **③ 잔금 — 지면 조판 1회.** 정량 게이트만. 유일한 정성 질문은 「조판이 뼈대를
  배신했는가」 — 배신 수리는 조판 몫, 뼈대를 고치려면 ②로 회귀한다.
정성 심사(IG4)는 시범 운용이다 — 임계 시작값 7.0(config review.threshold),
미달이어도 발행은 하되 보고에 「정성 미달 N점」 명기. 임계·차단 승격은 편집장 결정.

# 4. 산출물 생성
대본은 `templates/script-radio.md` 를 따라 `output/script/YYYY-MM-DD.md` 로 쓴다.
심층이면 아티팩트도 갱신한다.
수집 등재 시 각 사실의 **큰축 관측 태그**(지지/반증/조건형성/중립·소속 MA#)를 함께
판정해 `okf/axes.md` 점수판을 갱신한다. 축의 증류·재귀납(축을 되보는 절차)은 2b 절이 정본이다
(이슈 #35 — 종전의 「소급 재조사는 끝났고 이후는 증분이다」 문안을 그 절이 대체했다).

두괄식을 지킨다.
- 문서 최상단 「핵심」 블록을 먼저 갱신한다. 여섯 줄만 읽으면 결론이 끝나야 한다
- 각 섹션은 h2 바로 아래 결론 한 줄(`class="lede"`)로 시작한다. 표부터 내밀지 않는다

반복 억제를 지킨다.
- 사실은 1회. supersede 로 되살아난 것은 정정으로 다룬다
- 일정은 신규 등재일 / D-7 / 당일, 최대 3회
- 심층에서 배경을 다시 설명하지 않는다
- 신규가 없으면 `quiet_day` 항목을 쓴다. 반복으로 분량을 채우지 않는다

SOT 참조는 개념 ID 로 건다.

전시물 우선 원칙 (2026-08-12 확정): 웹판의 절 구성·전시물(1차자료 원문 재현+하이라이트+주석)·
줄글 상한·quiet day 강등 사다리·마크업 계약·PPTX/라디오 분업은 킷의
`templates/exhibit-first.md` 를 따른다 — 산출물 생성 전에 읽는다. 전시물마다 원문을
`sources/YYYY-MM-DD/` 에 저장하고, 발행 직전 `python tools/check_exhibits.py <웹판.html>`
(종료코드 0)을 반드시 실행해 결과를 `eval/YYYY-MM-DD.md` 에 기록한다. check는 존재·계약만
판정하므로 질(판형 일치·하이라이트)은 호당 전시물 1개 무작위 스팟체크로 따로 기록한다.

# 4b. 배포 — 공유 버전 재지정과 검증 (config schedule.publish_at 이후에만, 기본 07:45)

**배포의 첫 동작은 로그인 판정이다**: 내장 브라우저에서 `get_page_text` 로 claude.ai
로그인 여부(ⓐ)를 확인한다 — 세션이 풀려 있으면 공유 설정을 수동 안내로 강등하고
보고에 "재로그인 필요"를 남긴다. 조용히 지나가지 않는다.

**아티팩트를 갱신한 날에만 수행한다. 갱신하지 않았으면 건너뛴다.**
**publish_at(기본 07:45) 이전이면 수행하지 않는다. 시간표 절 참조.**

배포 묶음에는 오디오 원본 파일(WAV/MP3)을 반드시 첨부한다 — 아티팩트 뷰어는
iframe sandbox에 allow-downloads가 없어 ⋮ 메뉴의 다운로드 항목 자체가 사라진다
(2026-08-11 실측). 링크만 보내면 청취자는 저장할 방법이 없다.
런타임의 `downloads` capability 로도 못 푼다 — 선언하는 순간 플랫폼이 공개 공유를
거부한다 (deploy 422, 2026-08-12 실측). 허용 확장자에 wav·mp3 도 없다. 공개 배포
호에는 저장 버튼을 넣지 않는다.

웹판에 싣는 오디오는 **MP3(audio/mpeg)** 로 변환해 넣는다 (2026-08-13 개정 — 종전
표준이던 MP4(AAC)가 409 unscannable 의 **진범**으로 규명됐다, 아래 참조):
WAV 를 그대로 data URI 로 실으면 호가 10MB 를 넘어 첫 렌더가 눈에 띄게 느리다.
로컬 `output/audio/YYYY-MM-DD.wav` 원본은 배포 묶음용으로 남긴다.
비트레이트·채널은 **config 의 `tts.embed`** 가 정본이다(기본 48k·모노). 하드코딩하지 않는다.
발행 전 `tools/check_size.py`(D8)로 호 크기를 판정한다.
**공개 공유 거절(409 unscannable)의 원인은 크기가 아니라 코덱이다** — 2026-08-13 규명:
같은 데스크톱 앱에서 AAC(audio/mp4) 임베드 호는 2.7MB 여도 거절되고, 같은 내용을
MP3(audio/mpeg) 로 바꾸자 즉시 공유됐다. 이슈 #14 의 "발행 클라이언트 버전" 가설은
오진이었다(노트북 실패 호들이 전부 AAC 였던 것). 거절의 실체는
`PATCH /api/frame/perm/<id>` → 409 `{"reason":"unscannable"}` 이고, 화면 문구
("This version can't be shared publicly…")는 원인과 무관하다. 거절되면 문구를 옮겨
적지 말고 그 PATCH 응답 본문을 확인하고, 임베드 MIME 이 audio/mp4 인지부터 보라.
이식은 `python tools/embed_radio.py <웹판.html> output/audio/YYYY-MM-DD.wav <대본.md>`
한 번이면 된다 — WAV 를 주면 embed_radio 가 **MP3 변환을 스스로 한다**
(imageio-ffmpeg — SETUP 2절 선택 의존성). .mp4/.m4a 를 직접 주는 것은 금지다
(audio/mp4 임베드 = 공유 거절). imageio-ffmpeg 미설치·변환 실패면 경고 후
WAV 그대로 이식으로 강등된다 — 호가 무거워진 사실을 보고에 남긴다.
발행·공유 후 `python tools/check_publish.py YYYY-MM-DD` (E6) 로 익명 개통을 판정한다 —
비공개면 403 이 나와 실패한다 (새 아티팩트는 비공개가 기본값, 08.11 함정).
단 **403 이 곧 공유 OFF 는 아니다**: 회선에 CF 봇 차단이 걸리면 같은 403 이 챌린지
페이지로 온다. 그 경우 게이트는 종료코드 2(판정 불가)로 끝나고 어떤 처방도 내지 않는다 —
E5(MP3 재이식·재발행)를 따라가면 멀쩡한 호를 고치게 된다 (2026-08-15 실측, 이슈 #37).

호 마감 규칙 (2026-08-11 실측 실패에서): 한 호를 닫으면 **산출물 인계 보고
(아티팩트 링크·PPTX·아카이브 경로)를 먼저 완료**하고, 다음 호 작업은 그 다음에
시작한다. 마감과 다음 호 생성을 한 흐름에 섞지 않는다 — 받는 쪽에는 조용한
실패와 구분되지 않는다.

배포 URL 규칙 (2026-08-12 확정 — 날짜별 독립 출판): **호마다 새 아티팩트를 발행한다.**
전날 호의 URL은 건드리지 않는다 — 그 자체가 영구 아카이브다. 어제 URL에 오늘 호를
덮어쓰는 것은 금지 (독자가 받은 링크의 내용이 바뀌면 판형 자체가 무너진다).
발행 절차 (2026-08-12 순서 확정 — 게이트가 공유보다 먼저다):
① 새 파일로 Artifact 발행(새 URL)
② 게이트 통과 확인 — #5 전 항목 + C5 `python tools/check_formats.py <웹판.html> --expect-deck|--no-deck` 기본(정적) 검사. 플래그는 EVAL 판정으로 정한다 — 심층(덱 생산 호)이면 --expect-deck, 아니면 --no-deck. 파일 실존 추론에 맡기지 않는다 (이슈 #5: 덱을 만들고 기록을 빠뜨린 날을 '안 만든 날'로 오독)
③ **통과 후에만 아티팩트 페이지에서 Share → Anyone with the link 를 켠다** —
   새 아티팩트는 비공개 기본값이라 이 단계를 빠뜨리면 독자에게 404가 뜬다 (2026-08-12 실측).
   순서 근거: **공유를 켜는 순간 그 버전이 핀으로 잡힌다** (발표판 실측). 공유 ON 이전의
   수정은 자유롭고, 이후의 수정은 핀 이동 거절("This version can't be shared publicly...",
   재현됨·원인 미규명)에 노출된다 — 게이트 통과본을 켜면 핀 이동 경로 자체를 안 밟는다
④ 로그아웃 검증(아래 「검증」 절) + `python tools/check_formats.py <웹판.html> --check-links`
   — fmtbar 링크 전부를 익명 GET 으로 실개통 판정한다. 이 플래그는 발행·공유 완료
   이후에만 돌린다 (기본 실행은 정적 게이트 그대로 — 발행 전 게이트에 네트워크를 섞지 않는다)
⑤ URL을 `output/artifact-url-YYYY-MM-DD.txt`에 기록하고 배포 메시지에 당일 링크를
공지한다. **호 마감 = 당일 URL 기록 + `python tools/check_publish.py YYYY-MM-DD` 종료코드 0**
(2026-08-15 개정, 이슈 #37 — 종전 정의는 「URL 기록 완료」였다: URL 파일 한 줄이 남았을 뿐
독자가 그 링크로 오늘 호를 받는지는 아무도 판정하지 않은 채 호가 닫혔다).
종료코드 **2(판정 불가)면 마감이 아니다** — 봇 차단·네트워크로 게이트가 물어보지 못한 것이라
다른 회선에서 재판정한다. 재판정이 안 되면 **수동 확인 마감** 경로를 쓴다: 소유자가 브라우저에서
공유(Anyone with the link)와 핀 버전이 오늘 호인지 **직접** 확인한 뒤, run_log 종료 줄에
`"gates":[...,"check_publish:2"]` 와 아래 **형식 그대로**의 `reason` 을 적고 보고에
「수동 확인 마감」이라고 **명시**한다.

```
"reason": "수동 확인 마감 <YYYY-MM-DD HH:MM> <판정 불가로 끝난 게이트 이름 전부> — 확인 경위"
```

`check_run` 이 이 **형식**을 읽어 그 날짜를 닫는다 (2026-08-15 수리 — 검수 문제 4·9).
넷 다 있어야 한다: ⓐ 「수동 확인 마감」 표식 ⓑ 확인 시각 ⓒ 대신 확인한 게이트 이름 전수
ⓓ 경위 문구(표식·시각·게이트명을 뺀 15자 이상). 하나라도 없으면 종전대로 exit 1 이다.
**마감 정의는 하나다** — 호 마감 = 당일 URL 기록 + `check_run` 종료코드 0. 「수동 확인
마감」은 그 정의의 **예외가 아니라 0 에 이르는 두 번째 길**이고, 그 0 은 공유 상태에 대한
기계의 판정이 아니라 **형식 봉인**이다(황금률 ③ — 기계는 형식·봉인만 본다. 확인이 진짜였는지는
기계가 판정하지 않는다). 종전에는 이 상태가 기계에 없어 `:2` 로 끝난 날짜가 **영구히**
exit 1 이었고 — 문구만 「수동 확인 마감」으로 바뀌었을 뿐 마감은 여전히 불가능했다 — 그 압력이
`:2` 대신 `:0` 을 적게 만든다. 회선이 회복되면 `:0` 으로 재판정해 기록을 갱신한다.
발행된 아티팩트가 곧 영구 아카이브다 — 별도 사본 파일을 만들지 않는다.
발행 이력은 `output/artifact-url-*.txt` 가 유일한 소스다 — 이 문서에 URL을 적지 않는다.

기준 시점 규칙 (2026-08-12 실측 실패에서): 리드와 「세 개의 숫자」는 **가장 최신
실적 자료 기준**으로 정박한다 — 공시(DART) 확정치와 IR 잠정치가 시점이 다르면
최신 쪽을 앞세우되 등급을 명시한다 ("2Q IR 잠정, 공시 확정은 반기보고서").
그래프에는 넣은 최신 분기를 리드에서 빼먹으면 문서 앞뒤가 어긋난다.

라디오 이식은 `python tools/embed_radio.py <웹판.html> <오디오> <대본.md>` 로
한다 (오디오 data URI 교체 + 접힘 대본 재구성, 검증 실패 시 원본 보존).

2구역 구조 (2026-08-12 확정): 웹판은 「오늘의 뉴스」와 「자료실」(.zonebar 경계)로
나뉜다. 매일 재작성하는 것은 뉴스 구역뿐이다 — 리드, 오늘 새로 확인된 것, 세 개의
숫자, 라디오. 자료실(원문 표·관계 구조·SOT)은 새 사실이 판정될 때만 수정한다.
어제 것을 재편찬해 오늘 것처럼 내보내지 않는다 — 뉴스 구역이 비면 quiet_day로
비었다고 쓰는 것이 재탕보다 낫다.

공개 공유와 `Latest` 는 양립하지 않는다. 시스템이 막는다
("Can't switch to Latest while shared publicly"). 공유 링크는 지정된 버전에 고정되므로,
갱신 후 새 버전으로 다시 지정하지 않으면 링크를 여는 사람은 옛 내용을 계속 본다.

브라우저 선택 (2026-08-12 확정): **앱 내장 브라우저(Claude_Browser)를 우선 사용한다** —
사용자의 크롬을 점유하지 않는다. 내장 브라우저에 claude.ai 로그인이 돼 있으면 공유 핀
지정·디자인 조작까지 전부 내장 브라우저에서 한다. 로그인이 없으면(검증만 가능) 그때만
claude-in-chrome 으로 폴백하고, 폴백 사실을 보고에 남긴다.

**지정**
1. 내장 브라우저(Claude_Browser)로 연다. claude-in-chrome 폴백일 때만 `list_connected_browsers` 로 연결 확인
2. `output/artifact-url-YYYY-MM-DD.txt`(당일 파일)의 URL 로 navigate
3. Share 버튼 클릭 → `Shared version` 콤보박스 클릭
4. **드롭다운을 screenshot 으로 펼쳐 목록 전체를 읽는다.**
   `find` 가 돌려준 항목 하나로 판단하지 않는다. 발행이 여러 번 있었으면 버전이 여럿 쌓인다
5. **번호가 가장 큰 버전**을 고른다. 시간 표기("6m ago")로 고르지 않는다.
   같은 시각에 여러 버전이 찍힐 수 있다
6. `General access` 는 건드리지 않는다
7. **지정이 거절될 수 있다.** "This version can't be shared publicly. Publish a new
   version or change the shared version, then try again." 가 콤보박스 밑에 뜨면 그
   버전은 공개 대상이 아니다 — 핀은 옛 버전에 그대로 남는다 (2026-08-12, 두 아티팩트에서
   재현). 이때 조용히 넘어가지 말고 ⓐ 잠시 뒤 재시도 ⓑ 그래도 거절되면 보고에
   "핀 이동 실패 — 독자는 이전 버전을 본다" 라고 적는다. 갱신했다고 적지 않는다
8. 목록의 버전 번호는 **절대 식별자가 아니라 목록 안 위치값**이다 (호마다 20개 창).
   재발행할 때마다 기존 핀의 번호가 하나씩 내려간다 — 번호가 바뀐 것을 핀이 움직인
   것으로 읽지 않는다. 판정은 "번호가 가장 큰 것 = 최신" 으로 하고, 콤보박스가 보여주는
   경과 시간("just now")으로 교차 확인한다

**검증 — 반드시 한다**
소유자 화면은 언제나 최신을 보여주므로 그것으로 확인했다고 하지 않는다.
로그아웃 상태에서 공유 링크가 실제로 주는 내용을 확인한다.

1. 이 앱 내장 브라우저(`mcp__Claude_Browser__navigate`)로 아티팩트 URL 을 연다. 이 브라우저는 로그아웃 상태다
2. `javascript_exec` 으로 iframe 의 `src` 를 읽는다 (`*.frame.claudeusercontent.com/_f/...`)
3. 그 URL 을 `Invoke-WebRequest` 로 직접 받아 이번에 넣은 내용이 들어 있는지 문자열로 확인한다
   (예: `class="keypoints"`, 새로 추가한 문장, 고친 수치)
4. 없으면 4b 를 다시 수행한다
5. `python tools/check_formats.py output/web/YYYY-MM-DD.html --check-links` (종료코드 0) —
   fmtbar 의 각 링크(덱 포함)가 익명 독자에게 실제로 열리는지 HTTP 로 판정하고 결과를
   `eval/YYYY-MM-DD.md` 에 기록한다. 08.11호는 발표판이 private 404 인 채로 정적 게이트를
   통과했다 — 링크 존재와 링크 개통은 다른 판정이다. 실패("공유 OFF 의심")면 해당
   아티팩트의 Share 를 켜고 재실행한다

크롬이 연결돼 있지 않거나 로그인이 없으면 **조작하지 말고 보고에 남긴다.**
"공유 버전이 옛 버전에 고정돼 있음 — 수동 조작 필요" 라고 적는다.
검증을 하지 못했으면 "검증 미실행" 이라고 적는다. 확인한 척하지 않는다.

# 5. 게이트 — 산출물 전부에 건다

**아티팩트와 대본 둘 다 대상이다. 하나라도 빠뜨리지 않는다.**

| 산출물 | style-guard genre | tone-guard | 추가 게이트 |
|---|---|---|---|
| 웹판 | report | 적용 | headline-guard(헤드라인), insight-guard(리드) |
| 대본 | broadcast | 적용 | tts-guard(기호→발화, pace) |
| 슬라이드 | report | 적용 | 독자 언어 게이트 + publish-checklist A4 |
| 원장(OKF) | 적용 안 함 | 적용 안 함 | — |

`templates/tone-guard.yaml` 과 `templates/style-guard.yaml` 을 읽고
`templates/eval-rubric.md` 의 게이트 G1~G8 을 각 산출물에 통과시킨다.

- 수치는 완화하거나 반올림하지 않는다
- 1차자료 근거가 없는 문장에 단정 어미를 쓰지 않는다
- 사내 부서·팀·개인을 지목하지 않는다
- 클로징은 "앞으로 확인할 지표"까지만

**HTML 을 고쳤으면 렌더를 확인한다.** CSS 를 새로 넣었으면 특히 그렇다.
`li` 나 문단에 `display:grid` 를 걸면 그 안의 `<b>` 같은 인라인 요소까지 그리드 아이템이 되어
글자가 세로로 쪼개진다. 실제로 겪은 사고다. 레이아웃을 바꿨으면 screenshot 으로 눈으로 본다.

**게이트를 돌린 결과를 `eval/YYYY-MM-DD.md` 에 산출물별로 기록한다.**
통과 / 수정 후 통과 / 미통과 중 하나로 적고, 수정한 항목을 나열한다.
게이트를 돌리지 않았으면 "미실행" 이라고 적는다. 돌린 척하지 않는다.

**아티팩트를 갱신했는데 게이트 기록이 없으면 그 실행은 실패로 본다.**

**발행 전 체크리스트가 최종 관문이다** (2026-08-12 확정):
`templates/publish-checklist.md`의 A~E 전 항목을 발행 직전에 돌리고 `eval/YYYY-MM-DD.md`에
항목 번호별 통과/실패를 기록한다. 하나라도 실패면 발행하지 않는다.

기계 게이트는 손이 아니라 스크립트로 돌린다 — 발행 직전 필수 실행:
```
python tools/check_tables.py output/web/YYYY-MM-DD.html    # D6 표 판형 (종료코드 0 필수)
python tools/check_exhibits.py output/web/YYYY-MM-DD.html  # F1~F4 전시물 계약 (종료코드 0 필수, templates/exhibit-first.md 참조)
python tools/check_css_vars.py output/web/YYYY-MM-DD.html  # D4 미정의 CSS 변수 (종료코드 0 필수)
python tools/check_ledger.py                               # D7 원장 정합 — 매달린 참조·유효/대체 충돌·인덱스 신선도·큰축 3중 장부(④⑤)·증류/재귀납 봉인(⑥, 이슈 #35) (종료코드 0 필수)
python tools/check_size.py output/web/YYYY-MM-DD.html      # D8 호 크기 — 상한 초과 시 공개 공유가 거절된다 (종료코드 0 필수, config tts.embed.max_html_mb)
python tools/check_insight.py output/web/YYYY-MM-DD.html   # IG1 인사이트·헤드라인 — 축·I2·H6·I8·H9(화두 구체성, issue #18) 정적 판정, 판정 불가 항목은 경고로 남는다 (종료코드 0 필수, issue #15)
python tools/check_formats.py output/web/YYYY-MM-DD.html --expect-deck|--no-deck   # C5 판형 바 — EVAL 판정대로 플래그 명시, 덱 URL 반영 뒤 재실행 (종료코드 0 필수. `--check-links` 로 돌릴 때는 E6 와 **같은 3분법** — 0/1/**2 판정 불가**(CF 봇 차단), 2026-08-15 검수 문제 6: 종전에는 여기만 CF 를 「공유 OFF 의심」 1 로 찍어, E6 가 2 로 끝나는 같은 순간 실행자가 공유를 다시 켜고 재발행하러 갔다)
python tools/check_theme.py output/web/YYYY-MM-DD.html     # IG3 연간 주제 정렬 — 마지막 전체검수. eval/theme-날짜.md 에 화두·단신·절·전시물 단위별 정렬/보조/무관 판정 기록이 정본. 화두=정렬 필수, 무관 0건, 검수 메타 언어(규명·표본·주장 강도 등) 지면 0건 (종료코드 0 필수, 2026-08-13 지시 — 비정렬 에피소드는 노이즈, 검수 설명은 지면 오염)
python tools/check_review.py output/web/YYYY-MM-DD.html    # IG4 심사 봉인 — eval/review-YYYY-MM-DD.md 실존·3인×전차원 전수(루브릭 버전 스코핑 — v2 6차원, v1 기록 5차원)·평균≥임계(config review.threshold 우선, 기본 7.0)·지적 처리 칸 0빈칸·미달 시 재조판 1회→재심사. 점수는 심사 패널(templates/review-form.md)이 매기고 이 게이트는 봉인만 한다. 소급 심사는 --retro (종료코드 0 필수. 심사 대상의 정본은 중도금 뼈대 MD — templates/pipeline-three-stage.md)
#   3분법의 한계 (2026-08-15 명시 — 검수 문제 7): (a) 3분법을 쓰는 게이트는 E6 `check_publish` 와 E7 `check_formats --check-links` **둘뿐**이고 나머지는 종전대로 0/1 만 낸다 (b) CF 판정은 응답 본문·`cf-mitigated` 헤더의 **문자열 휴리스틱**이라 CF 가 문구를 바꾸면 다시 「공유 OFF」로 오진한다 (c) 이 갈래는 오진을 막을 뿐 **CF 회선에서 공유 상태를 판정해 주지 않는다** — 2 를 받은 호의 공유 여부는 미지로 남고, 다른 회선 재판정이나 「수동 확인 마감」(4b ⑤)만이 닫는 길이다
python tools/check_publish.py YYYY-MM-DD                   # E6 공유 확정 — 발행·공유 완료 뒤에만. api/frame 직접 판정(200+public, 무작위 uuid 대조군 자기검증) + 발행본 본문에 오늘 호 h1 대조 — 핀이 옛 버전·다른 호면 실패 (종료코드 0 필수, 이슈 #21 — 게이트는 발행본을 직접 본다. check_formats --check-links 도 링크의 발행본 실림(③)까지 판정한다. 종료코드 3분법 — 0 통과 / 1 판정 결과 실패 / **2 판정 불가**(CF 봇 차단·네트워크: 대조군이 챌린지면 대상 조회 전에 2 로 끝낸다) / 64 사용법 오류, 이슈 #37)
```
게이트를 돌리기 **전에** `eval/theme-YYYY-MM-DD.md` 판정표를 먼저 쓴다 — 각 단위가
연간 주제(profiles spine.annual_theme)와 어떻게 정렬되는지, '보조'는 왜 필요한 토픽인지
근거 한 줄씩. '무관' 판정이 나온 단위는 지면에서 빼고 나서 발행한다.
돌린 게이트와 종료코드는 run_log 종료 줄의 `gates` 필드에 그대로 옮긴다 (0-원장 절 —
게이트 기록이 없는 발행은 check_run.py 가 실패로 판정한다).
`check_formats` 는 4c(덱 발행)에서 fmtbar에 덱 URL을 넣은 **뒤** 한 번 더 돌린다 —
덱을 만들고도 웹판에 문을 안 달면 독자에겐 없는 것과 같다 (2026-08-11 실측).
덱을 만들지 않은 호(표준/간략 — 4c는 '선택, 심층인 날만')는 당일
`output/artifact-url-slides-YYYY-MM-DD.txt` 가 없으면 check_formats 가 프레젠테이션
세그먼트 요구를 자동 면제하므로 종료코드 0 으로 통과한다 (2026-08-12 수리 —
종전에는 무조건 실패라 '종료코드 0 필수'와 4c '선택'이 서로를 배제했다).
눈으로 훑는 게이트(A·B·C)와 달리 D 계열은 정적 검사가 가능하므로, 가능한 것부터
스크립트로 옮긴다. 새 반려가 정적으로 판정 가능하면 check_*.py 를 늘려라. 이 체크리스트는
2026-08-11~12 사용자 반려 전건을 항목화한 것이다 — 같은 지적이 다시 나오면
규칙 부재가 아니라 게이트 미실행이다.

# 4c. PPT — 클로드 디자인 (선택, 심층인 날만)

클로드 디자인(claude.ai 좌측 하단 「디자인」)의 Slides 템플릿을 쓴다. Max 구독에 포함, 별도 키 없음.
`output/ppt/YYYY-MM-DD-개요.md` 를 만들고(전 절 커버리지 — 압축본 금지, 2026-08-11 실측: 압축 개요를 넣으면 커버리지 부족),
프롬프트에 반드시 넣는 절대 규칙: 문안·숫자 한 글자도 변형 금지 / 파생값 계산 금지 / 시한부 서술의 단정화 금지 / 이미지 생성 금지 / 강의 덱 테마(Pretendard·크림 F5F1E8·잉크 1F2732·네이비 263A52·오렌지 D56643·표지 다크 17212B).
모델은 Fable 5. 생성 후 3종 오염 검사(사실 반전·파생값·단정 강화)를 돌린다 — 감마 실측에서 셋 다 발생했고 클로드 디자인은 통과했지만 매회 검사한다.
Share → PowerPoint(Universal fonts) 다운로드 → `output/ppt/` 보관. 다운로드가 .tmp 로 저장되면 zip 시그니처 확인 후 .pptx 로 복사.
덱도 날짜별 분리다 (2026-08-12 확정): 매일 클로드 디자인에서 **전일 파일을 복제해
새 파일(제일기획 아침 브리핑 YYYY-MM-DD)로 만든 뒤** 그 파일에서 갱신한다 —
새 파일에서 Publish as artifact 하면 새 아티팩트 URL이 나온다. 전일 덱 아티팩트는
그대로 보존하고 절대 덮어쓰지 않는다. 발행한 URL을 `output/artifact-url-slides-YYYY-MM-DD.txt`에
기록하고 **당일 웹판에 fmtbar 를 달아 프레젠테이션 링크에 당일 덱 URL을** 넣는다.
(판형 바는 덱 생산 호 전용이다 — 전환할 판형이 하나뿐인 표준·간략 호는 fmtbar 를
아예 넣지 않는다. 세그먼트 1개짜리 바는 죽은 버튼이라 check_formats 가 실패시킨다,
2026-08-13 편집장 반려.)
아카이브 호의 프레젠테이션 탭은 링크 없는 라벨(PPTX 배포본)로 남긴다.
**덱 아티팩트에도 발행 직후 Share → Anyone with the link 를 켠다** — 웹판만 켜면
fmtbar 링크가 남에게 404다 (2026-08-11 실측: 발표판이 private 인 채로 남아
링크조차 붙지 않았다). 링크를 넣은 뒤 `python tools/check_formats.py <웹판.html>`
(종료코드 0)로 판정하고 결과를 `eval/YYYY-MM-DD.md` 에 기록한다.
덱 구성은 **당일 웹판 구성을 미러링해 매일 재구성한다** (2026-08-12 유착 사고에서):
전일 슬라이드 승계 금지. 상시 자료는 '지난 호 안내' 1장으로만. 표지 헤드라인 = 당일 웹판 헤드라인.
덱 발행 이력도 `output/artifact-url-slides-*.txt` 가 유일한 소스다.
브라우저(claude-in-chrome)가 없으면 이 절은 건너뛰고 보고에 남긴다.

인포그래픽 원칙 (2026-08-11 세 번의 실측 실패에서):
- 순서도 장르 금지 — 다이아몬드·예/아니오·박스-화살표 사슬은 뭘 입혀도 CS 알고리즘으로 읽힌다
- 손글씨·스케치 테마 금지 — 두 번 시도 모두 조악했다
- 원칙: 절차가 아니라 사례가 주인공. 규칙은 캡션으로 내린다. 흐름은 띠·지류·밴드 같은 지면 은유로
- 빈 칸도 정보다 — "반증 아직 0건, 가장 기다리는 칸" 같은 빈자리를 지우지 않는다

독자 언어 게이트 (2026-08-11 관계도·분류·보류 3장 연속 반려에서):
- 날짜는 날짜로 표기 — "5.27" 같은 소수점 표기는 점수로 읽힌다. "5월 27일"로
- 명사 조각 금지 — "agentic 채택 5.27", "원인 출처 미확인, IR 원문 필요"는 편집실 메모다. 슬라이드 본문은 완전한 문장으로
- 내부 운영 체계(보강/반증/전환/무관 분류, "1/3" 카운터, "축으로 올린다" 규칙)는 본문 금지 — 필요하면 작은 캡션으로만
- 검사법: 슬라이드를 처음 보는 사람이 앞뒤 설명 없이 문장만 읽고 이해되는가. 안 되면 반려
- 취재 과정 언어 금지 (2026-08-12 재발에서): "~로 확인됐다", "일정 정정", "8.12 새벽 기준",
  "보도로만 알던" 같은 문구는 편집실 일지다. 기사 언어로: 사실 자체를 서술하고 출처는 srcline으로.
  확인·정정·조회 시각 기록은 원장(facts.jsonl)의 note 필드에만 남긴다

# 5b. 저장소 동기화 — 규칙이 바뀐 날은 반드시 한다 (원작자·포크 전용)
**원작 저장소에 쓰기 권한이 없는 설치본, 또는 zip 설치(.git 없음)는 이 절 전체를 건너뛰고
보고에 "저장소 동기화 해당 없음"이라고 남긴다.** 자기 포크가 있으면 포크로 push 한다.
이 문서(SKILL)나 `templates/`, `tools/`, `profiles/` 가 바뀌었으면, 루틴 마지막에
킷 저장소(https://github.com/INTEGRITY2077/cheil-briefing-kit)에 반영한다:
1. 이 문서를 킷의 `routine-SKILL.md` 로 복사한다 (경로는 원본 유지 — 설치자가 치환한다)
2. `git add -A && git commit -m "규칙 갱신: <한 줄 요약>" && git push`
3. push 실패(인증 등)면 조작하지 말고 보고에 "저장소 동기화 실패"라고 남긴다
routine-SKILL.md 를 레포에 동기화할 때는 「킷 위치」절의 로컬 절대경로를
반드시 `{{KIT_ROOT}}` 플레이스홀더로 바꿔서 커밋한다 — 원작자 경로가 배포되면
치환 실패 시 조용한 오작동이 된다 (2026-08-12 검출). 치환은 손으로 하지 않는다:
`python tools/sync_skill.py <마스터SKILL경로>` 를 킷 루트에서 실행하면 레포의
routine-SKILL.md 가 재생성되고, 절대경로·홈 경로 흔적이 남으면 종료코드 1 로 실패한다.
산출물(output/)과 개인 설정은 .gitignore 가 막는다 — 커밋 전 `git status` 로
.env·config.yaml 이 스테이징에 없는지 확인한다. 있으면 커밋하지 않는다.

# 6. 음성 — 사다리: supertonic → gemini → edge (2026-08-12 확정)
config의 `tts.engine` 을 본다. **1순위 `supertonic`** (로컬 ONNX, 키·네트워크 불필요):
`python tools/make_audio_supertonic.py output/script/YYYY-MM-DD.md`
— 기본 보이스는 `voices/anchor-f1f2-30.json`(실청 채택 블렌드), 합성 speed 1.05 고정
(배속은 웹판 플레이어 기본 1.3×가 담당 — tts-guard pace 절). 1인 대본(A: 줄만)이 표준이고,
영역 전환 접속사 규칙은 script-radio 템플릿을 따른다.
supertonic 실패(미설치·모델 다운로드 불가) 시 **2순위 `gemini`**:
`python tools/make_audio_gemini.py <대본> <출력.wav>` — `.env`의 GEMINI_API_KEY가
없으면 **3순위 `edge`** 강등(`tools/make_audio.py`). 어느 단으로 강등했는지 보고에 남긴다.
생성 후 파일 크기가 0 이 아닌지, 재생 길이가 대본 분량과 맞는지 확인한다.
`sapi` 면 ko-KR 음성을 탐지해 생성하고, 없으면 대본까지만 만들고 그 사실을 남긴다.
`notebooklm` 관련 참조물은 원작자 로컬 전용이다 — 정기 브리핑에 쓰지 않는다.

노트북LM 을 쓸 때의 함정 (2026-08-11 실측):
- "제일기획"을 "제일기업"으로 발음한 전례. 프롬프트에 발음 지시를 [가장 중요]로 넣는다
- "어~ 안녕하세요 청취자 여러분" 애드리브 오프닝 전례. 첫 문장을 문자열로 고정한다
- 길이 지시(2~3분)를 무시하고 5분 이상 생성한 전례. 대화상자의 「길이: 짧게」를 선택한다
- 요약 화면에서 연도를 2024로 틀린 전례. 오디오 검수 때 연도를 확인한다
프롬프트 원본은 `output/notebooklm/_맞춤설정_프롬프트.txt`. 실패 기록이 주석으로 붙어 있다.
생성 후 검수 없이 audio/ 로 옮기지 않는다: 첫 10초 청취, 수치 대조, 연도 확인.

# 7. 기록
- config `paths.ledger` 의 원장(기본값 `output/ledger/broadcast_log.jsonl` — 0절과 동일)에
  오늘 읽은 개념 ID 추가
- 방송한 개념에 `broadcast_at` 기록
- 심층이었으면 `okf/log.md` 에 한 줄
- `okf/index.md` 재생성
- `python tools/check_ledger.py` 종료코드 0 확인 (D7 원장 정합 — 매달린 참조 0건,
  deprecated 가 index 「유효」에 없음, 인덱스 신선도). 실패면 2절 미러 누락이다 —
  JSONL 에만 쓰고 끝낸 실행은 여기서 걸린다 (2026-08-12 이슈 #7)

# 8. 정리
- `stale_after` 지난 개념을 재확인 대상으로 표시
- 오디오는 `retention.audio_days` 초과분 삭제 — **파이썬으로 지운다**
  (`os.remove`, `Bash(python:*)` 허용 목록 안). 셸 `rm` 은 SETUP 5 허용 목록에
  없으므로 쓰지 않는다 — 무인 실행에서 권한 프롬프트가 뜬다. 삭제한 파일명을 보고에 남긴다

# 파일을 여럿 다룰 때
아티팩트 원본과 배포 사본이 따로 있으면 **편집한 파일과 발행한 파일이 같은지 확인한다.**
복사 후 발행하고, 발행 전에 두 파일의 크기나 해시를 대조한다. 실제로 어긋난 적이 있다.

# 보고
짧게 쓴다.
- **실행 실체 판정** — `python tools/check_run.py` 종료코드와 요지 (0-원장 절.
  산출물 부재면 이 보고는 실패 보고다 — 성공처럼 쓰지 않는다. 종료코드 확인은
  run_log 종료 줄에 `gates` 필드를 쓴 **뒤**에 한다 — 기록 전에 돌리면 자기 발행을
  게이트 기록 없음으로 실패 판정한다)
- EVAL 점수와 등급
- 신규 사실 건수, supersede 건수
- 게이트 결과 (산출물별)
- 아티팩트 갱신 여부, 공유 버전 재지정 결과, **로그아웃 검증 결과**, **배포 시각 (publish_at 이후인지)**
- 대본 경로와 예상 분량
- 사람이 할 것
- 확인하지 못한 소스가 있으면 그 사실과 이유

확인하지 못한 것을 확인한 것처럼 쓰지 않는다. 부분 실패는 부분 실패로 보고한다.
# 제품 이름 (2026-08-12 확정)
독자·수강생에게 부르는 이름은 세 가지뿐이다: **오늘의 제일기획 뉴스(웹판) · 라디오 · 프레젠테이션(슬라이드)**.
'화두판', '발표판', '특별판', '확정판' 같은 내부 작업명은 파일명·대화·보고 어디에도 다시 쓰지 않는다 —
내부 작업명은 결국 지면과 배포 문구로 샌다 (2026-08-11~12 반복 실측).

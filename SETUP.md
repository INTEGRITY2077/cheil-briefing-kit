...
이 항은 관찰 항목이 아니다. 1~5절은 로그인 없이도 진행할 수 있지만, **6절 3항
(아티팩트 발행·공유)은 이 항을 통과해야만 수행한다.** 미통과면 6절 3항은 건너뛰고
'미검증'으로 기록한다(6절·7절 참조).

**판정 절차 — 반드시 이 순서대로 한다:**
1. `mcp__Claude_Browser__tabs_create` 로 **새 탭을 만든다.** 이 경로만 브라우저 패널을
   실제로 화면에 띄운다(실측). `preview_start` 재사용(`reused: true`)이나 `tabs_select` 는
   패널을 펼치지 않으므로 이 단계의 대체가 되지 않는다.
2. `navigate` 로 https://claude.ai 를 연다.
3. `computer` 의 **screenshot 을 1회 반드시 찍는다.** `read_page`·`get_page_text` 는
   패널이 접혀 있어도 성공하므로 **판정 도구로 쓰지 않는다** — "사용자에게 보인다"의
   근거는 스크린샷뿐이다. 스크린샷이 "the Browser pane is not displayed" 류 오류로
   실패하면 그것이 곧 패널 접힘 판정이다 — `tabs_create` 부터 다시 하고, 그래도
   실패하면 사용자에게 패널을 펼치는 방법(앱 우측 상단의 브라우저 아이콘)을 안내한다.
4. 스크린샷에서 **"Sign in" 이 보이면 로그아웃 상태**다. 패널이 화면에 떠 있음을
   스크린샷으로 이미 확인한 상태이므로 이렇게 요청한다: "옆에 띄운 브라우저 패널에서
   claude.ai에 한 번 로그인해 주세요 — 공유 설정 자동화에 필요합니다. 로그인은 본인이
   직접 해야 하며 Claude는 비밀번호·인증코드를 대신 입력하지 않습니다."
5. 사용자가 로그인을 마쳤다고 하면 **다시 스크린샷을 찍어 재판정한다** — Sign in 이
   사라지고 로그인 후 화면이 보여야 통과다. 재판정 없이 통과 처리하지 않는다.

- 사용자가 지금 로그인할 수 없다고 하면: 1~5절은 계속 진행하되, 6절 3항이 '미검증'으로
  남고 7절 보고에 그 사실이 필수로 들어간다고 미리 고지한다.
...
3. Artifact 발행 → **발행 직후 Share "Anyone with the link" 켜기** (비공개 기본값 = 404 함정).
   **이 항은 0-3 로그인 확인을 통과했을 때만 수행한다.** 미로그인이면 이 항을 건너뛰고,
   시험 실행 결과에 실패도 통과도 아닌 **'미검증'** 으로 기록한다 — 로그인 후 첫 발행에서
   검증된다.
